(() => {
    ...
    window.gameState = {
        ...
        getExtensionUser: function() {
            ...
            // Blacklist of common UI words that are NOT usernames
            const blacklist = ['notifications', 'settings', 'profile', 'logout', 'login', 'account', 'menu', 'home', 'help'];
            ...
            for (const element of profileElements) {
                ...
                // - Not empty
                // - Reasonable length
                // - Not a blacklisted UI word
                if (text && 
                    text.length > 0 && 
                    text.length < 50 && 
                    !text.includes(' ') && 
                    !/^\d+$/.test(text) &&
                    !blacklist.includes(lowerText)) {
        ...
        eliminateImpossibleStates: function() {
            ...
            if (startCount !== finalCount) {
                console.log(`[PRUNE] Removed ${startCount - finalCount} duplicate states (${startCount} → ${finalCount})`);
            }
        },
